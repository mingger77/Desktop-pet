...
```
src/cyber-maid.py — DesktopPet(QWidget, DanceMixin, GameMixin, ChatMixin): 核心 + 状态机 + 事件 + 绘制
...
src/memory.md  — 长期记忆持久化文件（自动生成）

编译版（Nuitka / Inno Setup）自动将 config.json、env、memory.md 重定向至 `%APPDATA%\DesktopPet\`。
```
...
| 文件 | 职责 | 关键方法 |
|------|------|---------|
| `src/cyber-maid.py` | DesktopPet 核心 | `_setup_window()`, `_load_all_images()`（预加载双身份）, `_change_state()`, `_trigger_interaction()`, `_float_offset()`, `_switch_identity()`, 事件, `_tick()`, `paintEvent()` |
...
| `src/memory.md` | 长期记忆文件（自动生成） | — |
| `%APPDATA%\DesktopPet\` | 编译版数据目录 | 含 config.json / env / memory.md |
...
两种身份各 18 张，启动时一次性全部预加载：
- 女仆：`src/images/maid--{state}.png`
- 猫娘：`src/images/catgirl--{state}.png`

状态列表：`stand`, `lazy`, `cute`, `scaried`, `apologize`, `stone`, `scissors`, `paper`, `win`, `lose`, `draw`, `work`, `dance1` ~ `dance6`

`_load_all_images()` 将两种身份全部加载到 `self._all_images`，换装时仅做指针交换。
...
- 输出目录：源码版 `src/memory.md`，编译版 `%APPDATA%\DesktopPet\memory.md`
