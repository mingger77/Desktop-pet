...
PySide6 桌面宠物应用。无边框透明置顶窗口，使用 PNG 精灵图展示女仆/猫娘角色，通过状态机驱动交互和动画。支持右键菜单交互、AI 聊天（OpenAI SDK）、石头剪刀布游戏、跳舞动画、文档生成（PPT/Word/Excel/PDF）、换装主题切换。
...
| 文档生成 | python-pptx, python-docx, openpyxl, fpdf2 |
...
│   ├── 高级设置 (_show_advanced_settings)
│   ├── 文件上传 (_upload_file / _read_file_content)
│   └── API 请求 (_do_api_request / 工具调用)
├── work.py (文档生成工具)
│   ├── GENERATE_DOC_TOOL (function calling schema)
│   ├── handle_tool_call()
│   ├── generate_pptx()
│   ├── generate_docx()
│   ├── generate_xlsx()
...
chat.py ──import──→ work.py (GENERATE_DOC_TOOL, handle_tool_call)
...
main.py: QApplication()
  → DesktopPet.__init__()
    ...
    → _load_config()           // 从 config.json 加载 API key/identity
    → _setup_ui()              // 右键菜单策略
  → pet.show()
  → app.exec()
```
...
_do_api_request()
  ...
  → msg.tool_calls?
    ├── 否 → reply = msg.content
    └── 是 → 执行 tool_calls → handle_tool_call()
...
读取路径：
  ...
  ChatMixin._load_saved_output_dir()
  DanceMixin._load_saved_dances()

写入路径：
  ChatMixin._save_config()
  ChatMixin._save_output_dir()
...
| openai | OpenAI SDK（API 调用 + tool calling） | PyPI |
| python-pptx | PPTX 文档生成 | PyPI |
| python-docx | DOCX 文档生成 | PyPI |
| openpyxl | XLSX 文档生成 | PyPI |
| fpdf2 | PDF 文档生成（中文需 simhei.ttf） | PyPI |
| DeepSeek API | AI 聊天后端（兼容 OpenAI 协议） | 远程 API |
| simhei.ttf | 系统字体，PDF 中文渲染 | Windows 系统 |
...
工作模式启用时，API 请求附加 `GENERATE_DOC_TOOL` function calling schema。模型返回 tool_calls 后：

1. 将 assistant 消息（含 tool_calls）追加到 messages
2. 调用 `handle_tool_call()` 执行文档生成
3. 将 tool 结果消息追加到 messages
4. 第二轮 API 调用获取最终回复
...
### 【事实】dance.py 第 380-383 行 f-string 拼接错误
...
`f` 前缀仅作用于第一行字符串，第三行的 `{t['input_border']}` 不会被插值，显示为字面文本。
...
chat.py 中 `_show_advanced_settings()` 方法定义了完整的高级设置对话框，但过程中一直没有被任何地方调用。聊天对话框的功能按钮（工作模式/联网切换）直接在 `_show_chat_dialog()` 中以闭包形式实现。
