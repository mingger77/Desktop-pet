...
## 决策 11：删除文档生成功能（v1.1.0）

**日期**：2026-06-10

**问题**：文档生成（PPT/Word/Excel/PDF）功能复杂且使用率低，增加了维护成本。

**决策**：完全移除 work.py 及相关功能。

**影响**：
- work.py（196 行）整文件删除
- chat.py 删除工具调用（tool_calls）处理逻辑
- API 请求不再携带 tools 参数，简化为主线程→API→queue→UI 的单链流程
- requirements.txt 可移除 python-pptx/python-docx/openpyxl/fpdf2
- 工作模式按钮、位置配置按钮及对应逻辑一并删除

---

## 决策 12：跳舞导航 while 循环（v1.1.0）

**日期**：2026-06-10

**问题**：自定义编舞和已保存舞蹈的「返回」按钮直接回到主菜单，而非回到跳舞选择器。

**方案对比**：

| 方案 | 复杂度 | 效果 |
|------|--------|------|
| A | 低：_start_dance 加 while 循环 | 子页面返回时重新显示选择器 |
| B | 高：QStackedWidget 切换页面 | 选择器保持打开，切换子面板 |

**决策**：方案 A

**理由**：
- 改动最小，不需要重构对话框类
- 子页面方法返回 `"back"` 或 `"done"` 标志，_start_dance 据此决定是否继续循环

---

## 决策 13：始终显示工作菜单（v1.1.0）

**日期**：2026-06-10

**问题**：无 API key 时跳过工作菜单直接进入配置对话框，导致配置取消后无处可回。

**决策**：`_start_work()` 始终先显示工作菜单，「开始聊天」内部判断是否需要转向配置。

**影响**：
- 导航流统一：工作菜单 → 开始聊天 → 配置（无 key）/ 聊天（有 key）→ 取消 → 工作菜单
- 取消不再 fallthrough 到 stand

---

## 决策 14：循环次数 SpinBox → 按钮（v1.1.0）

**日期**：2026-06-10

**问题**：自定义编舞中循环次数使用 QSpinBox 下拉选择，与整体交互风格不一致。

**决策**：替换为三个并列 QPushButton（2次/3次/4次），选中态高亮。

**理由**：选项固定且数量少（3 个），按钮比下拉框更直观。

---

## 决策 15：初始化顺序修复（v1.1.0）

**日期**：2026-06-10

**问题**：`_load_images()` 在 `_load_config()` 之前调用，导致身份配置未读取时已用默认值加载错误图片。

**决策**：交换 `_load_config` 与 `_load_images` 的调用顺序。

**影响**：`_load_config` 先读取 config.json 中的 `identity`，`_load_images` 随后使用正确的身份前缀加载精灵图。
